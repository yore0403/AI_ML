...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        visit_corner = []
        return (self.startingPosition, visit_corner)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                visit_corner_list = list(visit_corner)
                if next_node in self.corners:
                    if next_node not in visit_corner_list:
                        visit_corner_list.append(next_node)
                
                successor = ((next_node, visit_corner_list), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    for item in corners: 
        if item not in state[1]:
            unvisited_corner.append(item)
            break

    hn = mindistance(unvisited_corner, state[0])
    return hn
