    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             captured).
        """
        noisyDistance = observation #distance pacman observe
        emissionModel = busters.getObservationDistribution(noisyDistance) #given noisyDistance -7~+7 distribution
        pacmanPosition = gameState.getPacmanPosition() #pacman distribution
        ...
        if noisyDistance == None: #ghost is captured by Pacman
            allPossible[self.getJailPosition()] = 1.0
        else:
            for p in self.legalPositions: #possible ghost positions(only consider these)
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[trueDistance] * self.beliefs[p] #count prob
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ghostPosState = self.setGhostPosition(gameState, oldPos)
            newPosDist = self.getPositionDistribution(ghostPosState)
            for newPos, prob in newPosDist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles): 
            self.particlesList.append(self.legalPositions[i%posNum])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: #ghost is captured by Pacman
            ...
            for p in self.particlesList:     
                sampleDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[sampleDistance] * self.getBeliefDistribution()[p]
        ...
        '''
        allPossible = util.Counter() #Counter
        if noisyDistance == None: #ghost is captured by Pacman
            allPossible[self.getJailPosition()] = 1.0
        else:
            for p in self.legalPositions: #possible ghost positions(only consider these)
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[trueDistance] * self.getBeliefDistribution()[p] #count prob
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
                allPossible = self.getBeliefDistribution()
        allPossible.normalize() 
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legalPositionsProducts = []       
        for i in itertools.product(self.legalPositions, repeat=self.numGhosts):
            legalPositionsProducts.append(i)
        #legalPositionsProducts = itertools.product(self.legalPositions, self.legalPositions )
        #print i for i in legalPositionsProducts,"xx "
        random.shuffle(legalPositionsProducts)
        posNum = len(legalPositionsProducts)
        for i in range(self.numParticles): 
            self.particlesList.append(legalPositionsProducts[i%posNum])
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None: #ghost is captured by Pacman
                allPossible[self.getJailPosition()] = 1.0

        for particle in self.particlesList:
            partial = 1.0
            for index, p in enumerate(particle):
                sampleDistance = util.manhattanDistance(p, pacmanPosition)
                partial *= emissionModels[index][sampleDistance]
        ...
        '''
        allPossible = self.getBeliefDistribution() #Counter
        print emissionModels,"ff"
        ...
        for p in self.legalPositions: #possible ghost positions(only consider these)
            ...
            for i in range(self.numGhosts):
                print emissionModels[i][trueDistance]
                partial *= emissionModels[i][trueDistance] #count prob
            
            print p ,partial
            allPossible[p] *= partial
        print "partial"
        print allPossible.argMax()
        if allPossible.totalCount() == 0:
            self.initializeParticles()
            allPossible = self.getBeliefDistribution()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particlesList:
            ...
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(state, i, self.ghostAgents[i])
